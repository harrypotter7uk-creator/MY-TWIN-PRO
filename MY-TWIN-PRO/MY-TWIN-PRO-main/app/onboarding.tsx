...
const cprStyles = StyleSheet.create({
  ...
  fill: { height: '100%', borderRadius: 2, position: 'absolute', left: 0, top: 0 },
  glow: { height: '100%', width: '100%', borderRadius: 2, position: 'absolute', left: 0, top: 0 },
...
export default function Onboarding() {
  ...
  const generateBothAvatars = async (uName: string) => {
    const gen = async (gender: TwinGender) => {
      try {
        const av = await apiPost('/api/avatar/generate', {
          user_id: userId, user_name: uName, gender, style: 'realistic', language: lang,
        });
        const url = av?.image_url || av?.url || '';
        return typeof url === 'string' && url.startsWith('http') ? url : null;
      } catch { return null; }
    };
    const [fUrl, mUrl] = await Promise.all([gen('female'), gen('male')]);
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      if (!analysisText) {
        analysisText = isAr
          ? 'يبدو أنك شخص عميق التفكير ومتوازن في قراراتك. تمتلك قدرة على التحليل والتعمق في الأمور. ستكون علاقتك بتوأمك الرقمي قوية ومثمرة وفريدة.'
          : 'You appear to be a deep thinker with balanced decision-making. You have a strong analytical mind. Your connection with your digital twin will be powerful, enriching, and unique.';
      }
      ...
      try {
        await Promise.all([
          ...
          apiPost('/api/fingerprint/generate', { user_id: userId, lang }),
        ]);
      ...
      <>
        ...
        {q.options.map((opt, i) => (
          <TouchableOpacity key={i} activeOpacity={0.75}
            ...
            onPress={() => handleAnswer(q.id, opt)} disabled={animatingStep || loading}>
    ...
    <View style={{ alignItems: 'center' }}>
      ...
      {birthReady && typedWelcome ? (
        <View style={[st.welcomeCard, { backgroundColor: colors.accentLight, borderColor: colors.accent }]}>
          ...
          {!typingDone && <View style={st.cursorRow}><View style={[st.cursor, { backgroundColor: colors.accent }]} /></View>}
        </View>
      ) : (
