...
async def lifespan(app: FastAPI):
    ...
    # ✅ Brain Scheduler – العقل الذي لا ينام (جميع المحركات الجديدة)
    try:
        from app.twin_state.brain_scheduler import brain_scheduler
        await brain_scheduler.start()
        logger.info("   ✅ Brain Scheduler started (3 cycles: light, hourly, daily)")
    except Exception as e:
        logger.warning(f"   ⚠️ Brain Scheduler skipped: {e}")
